...
- **Weapon hardpoints** come in two kinds — **pilot mounts** the captain fires for free, and **gunner mounts** that each need a gunner aboard — and each hardpoint has a **size: Light, Medium, Heavy, or Super Heavy**. A weapon only fits a mount of its size or larger, so you can't bolt six Siege Lances onto any hull. The **mix of mount sizes is rolled per ship** (and leans heavier on rarer hulls), so one frigate might come "2 light / 6 heavy" — a real warship — while another is "6 light / 1 medium / 1 heavy", better as a lightly-armed carrier. **Super-heavy mounts are very rare** and only on warships: a really good frigate might roll one (two is extremely uncommon); capitals are the super-heavy platforms. Pilot mounts usually run light/medium, but a lucky hull lets the captain wield the heaviest guns. The for-sale listing shows the mix so you can judge a hull before buying.
- **Weapons** come in sizes too: Light (Laser, Autocannon), Medium (Rocket Pod, Heavy Cannon), Heavy (Plasma Battery, Siege Lance), and Super Heavy (Mass Driver, Annihilator Beam), from **100 up to 55,000 attack**; a ship's attack is the sum of its manned guns. Buy and mount them at a **shipyard's Outfitting**.
- **Crew seats are labeled by role** — the **Captain** seat (yours), then **Pilots**, **Mechanics**, a seat per gun mount (a **"Gunner · Light/Medium/Heavy"** station once that mount has a weapon, or just an **open seat** while it's empty), and **passenger berths** — so it's clear where each crewmate goes. (Crew skill affecting combat is a planned next step.)
