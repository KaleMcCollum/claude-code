...
- **Weapon hardpoints** come in two kinds — **pilot mounts** the captain fires for free, and **gunner mounts** that each need a gunner aboard — and each hardpoint has a **size: Light, Medium, or Heavy**. A weapon only fits a mount of its size or larger, so you can't bolt six Siege Lances onto any hull. The **mix of mount sizes is rolled per ship** (and leans heavier on rarer hulls), so one frigate might roll "2 light / 6 heavy" — a real warship — while another rolls "6 light / 1 medium / 1 heavy", better as a lightly-armed troop carrier. The for-sale listing shows the mix so you can judge a hull before buying.
- **Weapons** come in sizes too: Light (Laser, Autocannon), Medium (Rocket Pod, Heavy Cannon), Heavy (Plasma Battery, Siege Lance), from **100 up to 10,000 attack**; a ship's attack is the sum of its manned guns. Buy and mount them at a **shipyard's Outfitting**.
